...
const STRUCTURE_NAME: { readonly [key: string]: string } = {
  ...
  "structure.watermill": "watermill",
  "structure.sawmill": "sawmill",
  "structure.quarry": "quarry",
  "structure.store": "store",
  "structure.warehouse": "warehouse",
  "structure.guardhouse": "guardhouse",
...
export function resolveToTileForge(composed: ComposedWorld): ResolvedWorld {
  ...
  for (let index = 0; index < cellCount; index += 1) {
    ...
    // Band classes -> package road types. Behavior 57 split trails (1,
    // dirtpath) from the road class (2); behavior 75 adds the STREET
    // class (3 -> the e2699cc "street" sett band, sl-0053/sl-0055):
    // settlement street webs wear it, through-routes keep the road band,
    // wilderness trails keep dirtpath.
    ...
    if (lane === 1) {
      ...
      road[index] = manifest.roadTypeByKey.get("street") ?? 0;
    }
