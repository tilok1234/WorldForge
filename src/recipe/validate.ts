...
export function validateRecipe(input: unknown): RecipeValidation {
    ...
    if (!isPlainObject(zones)) {
      ...
      if (!Array.isArray(entries)) {
        ...
        entries.forEach((entry, position) => {
          ...
          for (const key of Object.keys(entry)) {
            if (key !== "temperaturePermille" && key !== "moisturePermille" && key !== "elevationPermille" && key !== "anchor" && key !== "weight" && key !== "settlementFloor") {
          ...
          checkInteger(issues, entry, "settlementFloor", `$.zones.entries[${position}].settlementFloor`, 0, 8, false);
          if (layout === "anchors") {
  ...
  if (settlementStyle !== undefined) {
    if (!isPlainObject(settlementStyle)) {
      ...
      for (const flag of ["variety", "organicStreets", "narrowStreets", "urbanBlocks", "cityWalls"] as const) {
