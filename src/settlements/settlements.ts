...
/** Path-layer value for the TRAIL-class band: wilderness trails (the
 * dirtpath art). Behavior 74 briefly routed settlement streets here (the
 * sl-0049 ruling era, before the package had street art); behavior 75
 * moved them to STREET_BAND when the e2699cc cut shipped roadType 4. */
export const TRAIL_BAND = 1;
/** Path-layer value for the STREET-class band (behavior 75, sl-0053/
 * sl-0055): every settlement-written street surface — arms, the city
 * ring, house lanes, civic approaches, dock lanes — wears the package's
 * 10px sett-paved "street" band (roadType 4, road palette, blends into
 * cobble plazas by construction). The through-route keeps CITY_LANE and
 * wilderness trails keep TRAIL_BAND. Lamps key on {CITY_LANE,
 * STREET_BAND} — the b74 bandLaneMask workaround retired with this. */
export const STREET_BAND = 3;
    ...
    if (kind === "city" && rules.cityRingRadius > 0) {
      ...
      for (let d = -ring; d <= ring; d += 1) {
        for (const [rx, ry] of [
          ...
          if (cell !== -1 && isOpenLand(cell, grid, hydro)) {
            if (rules.narrowStreets) {
              // Street-class since behavior 75 (the ring is a street).
              routes.pathLayer[cell] = STREET_BAND;
