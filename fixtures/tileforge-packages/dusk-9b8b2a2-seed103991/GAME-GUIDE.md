...
- **The road, pier, fence and wall layers are typed**: one byte per cell —
  `mappings.roadTypes` (1 paved road, 2 dirt path — a narrow trail, 3 ruined
  road, 4 street — a 10 px sett-paved town band between dirt path and road;
  it wears the road palette so it blends into cobble plazas, 5 gravel way —
  a 12 px loose-gravel country highway, 6 flag way — a 14 px big-slab
  flagstone highway grander than street, 7 corduroy — a 10 px road of logs
  laid ACROSS the travel axis for swamp/frontier routes, 8 threshold — a
  kerbed sett MOUTH pad at road width; place ONE cell of it where the road
  class switches and the width step reads deliberate, like a forecourt
  entrance), `mappings.pierTypes` (1 pier — narrow, posted; 2 boardwalk — wide
  ...
  family using the shared mask. **Where road types mix, the shared cell takes
  the WIDER family** (road / ruined road / threshold 16 px over flag way 14
  over gravel way 12 over street / corduroy 10 over dirt path 7): a path teeing into
  a road works because the T cell is road-typed; a path CROSSING a road keeps
  the crossing cell road-typed too — a narrow-typed cell inside a wider run
  visually severs it (the wider band's open face steps down mid-run).
  **Class switches on a STRAIGHT run render a hand-authored ROAD JOINT
  automatically** (`mappings.roadJoints`): the forge substitutes the joint
  tile — shoulder taper steps, laid end courses, header logs, gravel
  spill — at the switch cell in BOTH the app and the exported map; the
  grid byte is untouched (the joint is pure render substitution). Runtime
  consumers replicate the same rule: a straight-run road cell whose
  neighbor's class ranks EARLIER in `roadJoints.classes` draws the joint
  tile for (A = neighbor, B = cell) with the orientation named by the
  neighbor's side — exactly one side of any switch fires. Switches AT a
  junction/corner keep the wider-class rule above. Optional flourishes: a
  THRESHOLD cell (type 8) makes a switch ceremonial (a kerbed forecourt
  mouth), and laddering through the middle class (road → street → dirt
  path) splits one big step into two small ones.
  Road and pier types grant walkability the same
  way in section 3 (fences always block), but only paved road reads as a
  bridge — keep paths, streets and ruined roads on land. Ruined-road potholes are
  transparent and show the terrain beneath.
- **Network runs are 1 cell wide by contract.** A net16 tile fills its center
  ...
  material to cobble (`fill.cobble` reuses the road palette, so road bands
  blend straight into it). For a road that crosses or enters a plaza, keep the
  road cells' material cobble as well — the band then reads as the marked lane
  across the square. Wide water works the same way: paint the water material
  ...
  corridor doctrine replaced. The road layer itself remains fully
  supported (restored 2026-07-30 after a brief retirement) for authored
  lane/trail/bridge marks — this recipe is about route DISPLAY, where
  the bare corridor wins. The route hierarchy, one recipe per class: trail = dirt path
  ...
  = the STREET band (road type 4 — the 10 px sett-paved band built for
  settlement webs, 2026-07-31; keep pure diagonal steps out of the web —
  band ports are orthogonal, so emit L-shaped step pairs) with plazas and
  big squares as cobble AREAS · country highway = the GRAVEL WAY band
  (type 5 — loose gravel, a worked rural trunk route) · stone highway =
  the FLAG WAY band (type 6 — big flagstone slabs, grander than street;
  processional ways, citadel approaches) · swamp / frontier causeway =
  the CORDUROY band (type 7 — cross-laid logs over bog and mire) · a road
  CLASS SWITCH mid-route = nothing to place — straight-run switches render
  the hand-authored ROAD JOINT automatically (section 2.4); add one
  THRESHOLD cell (type 8) only when the switch should read ceremonial, or
  ladder road → street → dirt path through the middle class · major river crossing = a bridge STRUCTURE
...
4. any road-layer type present (road, dirt path, ruined road, street, gravel
   way, flag way, corduroy, threshold) → walkable. Any
