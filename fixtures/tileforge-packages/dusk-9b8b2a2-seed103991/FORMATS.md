...
Top level: `formatVersion` (1), `generator`, `sourceCommit` (the tileforge
git commit this package was built from — consumers pin package identity on it;
null only in unprovenanced in-app exports), `contentHashes` (SHA-256 hex of
every other package entry, keyed by zip path, written by the CLI publisher at
build time — null in in-app exports; verify these at intake), `projectSeed`,
`tileSize` (32), `projection` ("orthographic-top-down"), `compatibilityMode`
("universal" | "coordinated"), `style` (the forge sliders),
`familyOverrides`, `palette` (role name -> hex, e.g.
`"water.foam": "#b8e0f2"`), `families`, `mappings`.
...
family key: 1 road, 2 dirtpath, 3 ruinedroad, 4 street, 5 gravelway,
6 flagway, 7 corduroy, 8 threshold), `minimap` (material id ->
...
5 cavewall, 6 dungeonwall), `roadJoints` (the road-class
transition contract: `classes` in seniority order, `pairs` with per-pair
`codeBase`, `code` = pairIdx*4+orient, `orients`, and the placement
`rule` — see section 2.4), `rampTypes` + `rampDirs` (ramp layer byte =
...
- **net16** (ports): N=1 E=2 S=4 W=8 from the network's own layer;
  **out-of-world does NOT connect** (runs end in a closed cap at world edges).
  ...
  `16-w/2 .. 16+w/2-1`, street 11–20 (fixed 10 px), gravel way 10–21 (12 px),
  flag way 9–22 (14 px), corduroy 11–20 (10 px), threshold the road band
  (its pad bulge never reaches the tile edge), river its band ± damp rim.
- **joint codes** (the road_joint tileset is mask-free): code =
  `pairIdx*4 + orient` where pairIdx enumerates i<j over
  `mappings.roadJoints.classes` and orient is 0 A-west / 1 A-east /
  2 A-north / 3 A-south (A = the senior class's side). Placement rule in
  section 2.4; ids `network.roadjoint.<a>-<b>.<aw|ae|an|as>.variant_##`.
...
- The `road` layer mixes gids from nine tilesets (road / dirt_path /
  ruined_road / street_paving / gravel_way / flagstone_way / corduroy_road /
  road_threshold / road_joint — the joint substitutes at straight-run class
  switches per section 2.4), the `pier` layer from three (pier / boardwalk / jetty), the
...
produce byte-identical atlases and manifests (verified browser vs headless
CLI), except the manifest's provenance fields (`sourceCommit`,
`contentHashes`) which only the CLI publisher writes — in-app exports carry
null there. Treat the package as a build artifact — regenerate rather than
hand-edit.
