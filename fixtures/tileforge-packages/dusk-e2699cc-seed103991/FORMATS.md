...
family key: 1 road, 2 dirtpath, 3 ruinedroad, 4 street), `minimap` (material id ->
...
- **net16** (ports): N=1 E=2 S=4 W=8 from the network's own layer;
  **out-of-world does NOT connect** (runs end in a closed cap at world edges).
  ...
  `16-w/2 .. 16+w/2-1`, street 11–20 (fixed 10 px), river its band ± damp rim.
...
- The `road` layer mixes gids from four tilesets (road / dirt_path /
  ruined_road / street_paving), the `pier` layer from three (pier / boardwalk / jetty), the
