...
- **The road, pier, fence and wall layers are typed**: one byte per cell —
  ...
  it wears the road palette so it blends into cobble plazas), `mappings.pierTypes` (1 pier — narrow, posted; 2 boardwalk — wide
  ...
  the WIDER family** (road or ruined road over street over dirt path): a path teeing into
  ...
  visually severs it (the wider band's open face steps down mid-run). Never
  butt-join a type switch mid-run either: switch types at a junction cell, or
  end the road and start the path one cell later (a deliberate trailhead gap).
...
- **Network runs are 1 cell wide by contract.** A net16 tile fills its center
  ...
  big squares as cobble AREAS · major river crossing = a bridge STRUCTURE
...
4. any road-layer type present (road, dirt path, ruined road, street) → walkable. Any
